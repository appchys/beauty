  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        console.log('JWT callback - token actualizado:', { email: user.email, role: user.role });
      }
      return token;
    },
    async session({ session, token }) {
      if (token && session.user) {
        ...
        console.log('Session callback - sesión actualizada:', { email: session.user.email, role: session.user.role });
      }
      return session;
    },
  },
